...
export function Root() {
	...
	const {mutateAsync, isLoading, invalidateRelated} = useMutateDocument<Card>(FirestoreMetadata.cardsCollection.name, 'add');
	...
	return (
		<Form customStyles={utilStyles.grid} onSubmit={onSubmit} fields={(form) => <>
			<FieldRow>
			...
			<FieldRow>
				<SubmitButton button={{
					disabled: isLoading,
				}} group={{ position: 'right' }}>Create</SubmitButton>
			</FieldRow>
		</>} initialValues={{
			word: '',
			deck: '',
